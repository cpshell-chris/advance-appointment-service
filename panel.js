(function () {
  ...
  function getPerformedJobs(roData) {
    ...
    return jobs.filter((j) => j.authorized === true);
  }

  function getDeclinedJobs(roData) {
    const jobs = roData?.jobs ?? [];
    return jobs.filter((j) => j.authorized === false);
  }
    ...
    panel.innerHTML = `
      ...
      <div class="aa-body">
        ...
        <div class="aa-card">
          ...
          <div class="aa-hint" style="margin-bottom:10px;">Declined today — add as notes for next visit.</div>
        ...
        <div class="aa-card">
          ...
          <div class="aa-hint" style="margin-bottom:8px;">Notes for the next visit (loaner, concerns, preferences, etc.)</div>
          <textarea
            class="aa-textarea"
  ...
  function buildAppointmentNotes(roData) {
    const performedJobs = getPerformedJobs(roData);
    const declinedJobs = getDeclinedJobs(roData);
    ...
    try {
      ...
      const response = await fetch(`${CLOUD_RUN_URL}/appointments`, {
        ...
        body: JSON.stringify({
          ...
          notes: buildAppointmentNotes(ro),
